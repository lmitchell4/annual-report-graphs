...
seg_subset <- subset(df_wq_three, df_wq_three$Sign == '<')
...
for (i in seq(length(analytes))){
  ...
  if (analytes[i] != 'SpCndSurface') {
    y_max <- max(df_wq_three_filt$Value, na.rm = TRUE)
  } else {
    y_max_int <- max(df_wq_three_filt[df_wq_three_filt$Region %in% int_delta,]$Value, na.rm = TRUE)
    y_max_out <- max(df_wq_three_filt[!df_wq_three_filt$Region %in% int_delta,]$Value, na.rm = TRUE)
  }
  ...
  out <- by(data = df_wq_three_filt, INDICES = df_wq_three_filt$Region, FUN = function(m) {
    ...
    if (cur_region == "Central Delta"){
      ...
      p <- ggplot() +
        ...
        geom_line(m, mapping = aes(Monyear, Value, group = Station, colour = Station), size = 1.1) +
        geom_point(m, mapping = aes(Monyear, Value, group = Station, colour = Station, shape = Station), size = 3) +
        blank_theme +
        theme(
...
for (i in seq(length(analytes))){
  ...
  graph <- marrangeGrob(grobs = as.grob(out), ncol=2, nrow=3, top=textGrob(plt_names[i],gp=gpar(fontsize=16, fontface='bold')))
